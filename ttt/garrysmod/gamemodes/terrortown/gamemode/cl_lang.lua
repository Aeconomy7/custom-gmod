...
function LANG.GetRawTranslation(name)
   if not cached_active then return nil end
   return rawget(cached_active, name) or (cached_default and rawget(cached_default, name))
end
...
function LANG.GetUnsafeLanguageTable() return cached_active or {} end
