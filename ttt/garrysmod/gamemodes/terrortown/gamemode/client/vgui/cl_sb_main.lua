...
function PANEL:Init()
    ...
    t:SetGroupInfo(GetTranslation("terrorists"), Color(66, 179, 255, 100), GROUP_TERROR)
    ...
    if gameloop.IsDetectiveMode() then
        t = vgui.Create("TTTScoreGroup", self.ply_frame:GetCanvas())
        t:SetGroupInfo(GetTranslation("sb_mia"), Color(255, 224, 102, 80), GROUP_NOTFOUND)
        self.ply_groups[GROUP_NOTFOUND] = t

        t = vgui.Create("TTTScoreGroup", self.ply_frame:GetCanvas())
        t:SetGroupInfo(GetTranslation("sb_confirmed"), Color(102, 187, 106, 100), GROUP_FOUND)
...
-- Great Sea color palette
local colors = {
    bg  = Color(10, 20, 35, 235),       -- dark navy (matches site background)
    bar = Color(255, 224, 102, 255),    -- gold (#ffe066)
}
...
function PANEL:Paint()
    ...
    -- Server name is outlined by gold area
...
function PANEL:ApplySchemeSettings()
    ...
    self.hostdesc:SetTextColor(Color(207, 233, 255, 255))   -- #cfe9ff
    self.hostname:SetTextColor(Color(10, 20, 35, 255))      -- dark navy on gold bar
    self.mapchange:SetTextColor(Color(207, 233, 255, 255))  -- #cfe9ff

    local sorting = cv_ttt_scoreboard_sorting:GetString()
    local highlight_color = Color(255, 224, 102, 255)       -- gold for active sort column
    local default_color   = Color(207, 233, 255, 255)       -- #cfe9ff for inactive
