...
if SERVER then
    ...
    resource.AddFile("sound/WHAT-THE-FU-BOOM.mp3")
end
...
function ENT:SetDetonateTimer(length)
    ...
    -- Play a warning sound 1 second before explosion
    if SERVER then
        local warnTime = math.max(length - 1, 0)
        timer.Simple(warnTime, function()
            if IsValid(self) and self:GetArmed() and self:GetExplodeTime() > CurTime() then
                -- Play the sound at the C4's position, audible within the explosion radius
                sound.Play("WHAT-THE-FU-BOOM.mp3", self:GetPos(), 130, 100, 1)
                -- Arguments: sound path, position, volume, pitch, sound level (1 = max, 0 = silent)
            end
        end)
    end
end
