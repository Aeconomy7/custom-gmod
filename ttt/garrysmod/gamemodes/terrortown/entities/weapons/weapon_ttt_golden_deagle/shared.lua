...
local function IsInnocentRole(role)
	local inno = (ROLE_INNOCENT and role == ROLE_INNOCENT)
		or (ROLE_SURVIVALIST and role == ROLE_SURVIVALIST)
		or (ROLE_DETECTIVE and role == ROLE_DETECTIVE)
		or (ROLE_SHERIFF and role == ROLE_SHERIFF)
		or (ROLE_DEPUTY and role == ROLE_DEPUTY)
		or (ROLE_SPY and role == ROLE_SPY)
		or (ROLE_WRATH and role == ROLE_WRATH)
		or (ROLE_LYCANTHROPE and role == ROLE_LYCANTHROPE)
		or (ROLE_PRIEST and role == ROLE_PRIEST)
	print("[GDEAGLE] IsInnocentRole(" .. role .. "): " .. tostring(inno))
	return inno
end

-- support for TTT Custom Roles
local function IsTraitorRole(role)
	local traitor = (ROLE_TRAITOR and role == ROLE_TRAITOR)
		or (ROLE_MESMERIST and role == ROLE_MESMERIST)
		or (ROLE_THRALL and role == ROLE_THRALL)
		or (ROLE_NECROMANCER and role == ROLE_NECROMANCER)
		or (ROLE_ZOMBIE and role == ROLE_ZOMBIE)
		or (ROLE_SERIALKILLER and role == ROLE_SERIALKILLER)
	print("[GDEAGLE] IsTraitorRole(" .. role .. "): " .. tostring(traitor))
	return traitor
end

local function IsInKillTeam(ply)
		...
		if SERVER then
			...
			hook.Add("EntityTakeDamage", title, function(ent, dmginfo)
				if (IsValid(ent) and ent:IsPlayer() and dmginfo:IsBulletDamage() and dmginfo:GetAttacker():GetActiveWeapon() == self) then
					...
					if ((TTT2 and owner:HasEquipmentItem("item_ttt_golden_bullet")) or ((killMode == 0 or killMode == 2) and IsInKillTeam(ent)) or ((killMode == 1 or killMode == 2) and not AreTeamMates(owner, ent))) then
