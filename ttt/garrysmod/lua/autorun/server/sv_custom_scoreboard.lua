...
local function GS_GetPlayerData(steamid64)
    ...
    local titleRow = sql.QueryRow([[
        SELECT A.name, A.stat_type, A.role_type, A.team_type
    ...
    return level, title, stat_type, role_type, team_type
end
...
local function TransmitPlayerDataToClient(targetClient, ply)
    ...
    local level, title, stat_type, role_type, team_type = GS_GetPlayerData(sid64)
    ...
    net.Start("GS_SendLevelTitle")
        ...
        net.WriteString(role_type)
        net.WriteString(team_type)
